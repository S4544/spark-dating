...
async function authenticate(req, res, next) {
  ...
  const r = await pool.query("SELECT user_id FROM tokens WHERE token=$1", [authHeader.slice(7)]);
  if (!r.rows[0]) { res.status(401).json({ success: false, message: "Invalid token. Please log in again." }); return; }
